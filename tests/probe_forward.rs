...
fn probe_software_forward_seek_moves_video() {
    ...
    let (mut video, mut audio) = ctx.create_video(&test_file(TEST_WEBM)).expect("open webm");
    ...
    // Forward seek from ~0.5s to 2.5s.
    ...
    while Instant::now() < deadline {
        ...
        if audio_peak > 2.0 && video_peak > 2.0 {
            break;
        }
    }
    eprintln!(
        "[FWD] post-seek {:.2}s: video_peak={video_peak:.3}s audio_peak={audio_peak:.3}s",
        t0.elapsed().as_secs_f64()
    );
    assert!(
        video_peak > 1.8,
        "video never reached target (video_peak={video_peak:.3})"
    );
    assert!(
        audio_peak > 1.8,
...
#[test]
#[ignore = "slow 45s deep-seek probe (decodes 45s of unindexed WebM video)"]
fn probe_software_forward_deep_seek_moves_video() {
    let ctx = SoftwareContext::new().expect("ffmpeg init");
    let (mut video, mut audio) = ctx.create_video(&test_file(TEST_WEBM)).expect("open webm");
    let rx = video.frame_receiver();
    let mut buf = [0f32; 8192];
    ...
    eprintln!(
        "[FWD-DEEP] post-seek {:.0}s: video_peak={video_peak:.3}s audio_peak={audio_peak:.3}s",
        t0.elapsed().as_secs_f64()
    );
    assert!(
        video_peak > 40.0,
        "video never reached 45s target (video_peak={video_peak:.3})"
    );
    assert!(
        audio_peak > 44.0,
        "audio never recovered after forward seek: still {audio_peak:.3}s"
